...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, z=None, y=None, **kwargs):
        ...
        if z is not None:
            ...
            z_proj = self.proj(z)
            # print("z proj shape: ", z_proj.shape, emb.shape, flush=True)
        ...
        for module in self.input_blocks:
            ...
            hs.append(h)
        # print("h before middle shape: ", h.shape, flush=True)
        h = self.middle_block(h, emb)
        # print("h after middle shape: ", h.shape, flush=True)
        for module in self.output_blocks:
            cat_in = th.cat([h, hs.pop()], dim=1)
            h = module(cat_in, emb)
        h = h.type(x.dtype).float()
        return self.out(h)
